...
export default () => ({
  ...
  health: {
    memoryHeapThreshold: parseInt(process.env.HEALTH_MEMORY_HEAP_THRESHOLD || '500', 10) * 1024 * 1024, // Convert MB to bytes
    diskThresholdPercent: parseFloat(process.env.HEALTH_DISK_THRESHOLD_PERCENT || '0.9'),
    mongodbTimeout: parseInt(process.env.HEALTH_MONGODB_TIMEOUT || '5000', 10),
  },
});
